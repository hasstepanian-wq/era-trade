...
InfinityFree не разрешает CryptoPro CSP, поэтому подпись запросов будет жить
на отдельном VPS. Минимальный сервер:

- **Beget VPS Start** (~200 ₽/мес) или Selectel.
- Ubuntu 22.04 + установка КриптоПро CSP + контейнер на Go или Java.
- Endpoint `POST /sign` принимает `{ "payload": "<строка>" }`, отдаёт
  `{ "signature": "<base64url-PKCS7>" }`.
